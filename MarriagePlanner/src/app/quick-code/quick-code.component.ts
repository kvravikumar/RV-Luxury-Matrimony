...
export class QuickCodeComponent {
  ...
  ngOnInit() {
    ...
    sessionStorage.setItem("ActivePage", "QuickCode");
  ...
  onFormSubmit(Errortemplate:TemplateRef<any>) {
    ...
    this.CreateQuickCode(user,Errortemplate);
  ...
  CreateQuickCode(quickCode: QuickCode,Errortemplate:TemplateRef<any>) {
    ...
    if (this.PageTitle == "New QuickCode") {
      ...
      this.quickCodeService.CreateQuickCode(user).subscribe((result) => {
        if (result.status == "success") {
          ...
          this.SuccessMassage = '';
          this.PopupErrorMessage = result.message;
          this.SpinnerService.hide(); 
          this.ErrorModal(Errortemplate)
        }
      }, error => {
        this.data = true;
        this.SuccessMassage = '';
          this.PopupErrorMessage = error.message;
          this.SpinnerService.hide(); 
          this.ErrorModal(Errortemplate)
      });
    }
  ...
  onEditClick(event, index: number,Errortemplate:TemplateRef<any>) {
    this.PopupErrorMessage = "";
    this.SuccessMassage = "";

    if (this.PageTitle == "Update QuickCode")
    {
      this.PopupErrorMessage = "Page already in edit mode.";
      this.ErrorModal(Errortemplate);
    }
    else {
      this.mode = "edit";
  ...
  ErrorModal(Errortemplate: TemplateRef<any>) {
